...
class ModelTester:
    ...
    def run(self) -> dict:
        ...
        train_precision = precision_score(
            self._train_true_y, train_pred_y, average=_avg
        )
        train_recall = recall_score(self._train_true_y, train_pred_y, average=_avg)
        train_specificity = train_tn / (train_tn + train_fn)

        # Compute testing stats
        test_pred_y = self._estimator.predict(self._test_x)
        print(self._test_true_y)
        print(test_pred_y)
        cm = confusion_matrix(self._test_true_y, test_pred_y)

        test_tn, test_fp, test_fn, test_tp = cm[0][0], cm[0][1], cm[1][0], cm[1][1]

        test_precision = precision_score(self._test_true_y, test_pred_y, average=_avg)
        test_recall = recall_score(self._test_true_y, test_pred_y, average=_avg)
        test_specificity = test_tn / (test_tn + test_fn)
        ...
        self._model_report |= {
            ...
            "train_accuracies": train_accuracies,  # type: list[tuple[str, float]]
            ...
            "test_accuracies": test_accuracies,  # type: list[tuple[str, float]]
